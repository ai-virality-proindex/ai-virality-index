...
| 31 | Feb 19, 2026 | Session 31 — llms.txt for AI Visibility ✅. Brainstorming: evaluated 3 approaches (static+dynamic, MCP server, JSON-LD). Chose Approach A: static manifest + dynamic data endpoint. (1) Created web/public/llms.txt — static Markdown manifest with project description, 7 models, 6 components, Trading+Content weight formulas, links. (2) Created web/src/app/llms-full.txt/route.ts — dynamic Next.js route generating Markdown with live scores from Supabase (vi_trade, vi_content, delta7, component breakdown per model). ISR 1h cache. (3) Updated sitemap.ts — added /llms.txt + /llms-full.txt + /blog to sitemap. (4) Updated robots.ts — explicitly allowed /llms.txt + /llms-full.txt. (5) Design doc: docs/plans/2026-02-19-llms-txt-design.md. Decision: reveal weight formulas (transparency builds trust), hide normalization internals (EWMA/quantile thresholds). Build OK (55/55 pages). | Verify on Vercel after deploy |
