...
| 25 | Feb 18, 2026 | Session 25 — Task 9.3: Component overhaul (10 subtasks). (1) Fixed GitHub delta bug: _get_previous_metrics uses lt(date)+order desc instead of eq(yesterday). (2) Replaced Q(Arena Elo) with D(Dev Adoption = npm+PyPI downloads) everywhere — config.py weights (Trading: 0.18T+0.28S+0.15G+0.12N+0.15D+0.12M, Content: 0.25T+0.35S+0.05G+0.20N+0.05D+0.10M), index_calculator.py COMPONENT_SOURCES, all frontend files. (3) New DevAdoptionCollector (etl/collectors/devadoption.py) fetching npm Registry + PyPI Stats APIs. (4) Added devadoption_packages to models_config.yaml + seed script. (5) Added Trends Redis caching (etl/cache.py): cache on success, fallback to cache on 429 failure. (6) Deleted dead market.py (Polymarket). (7) Updated 12+ frontend files: page.tsx, docs, pricing, stripe, BreakdownRadar, breakdown API, model pages. (8) Updated CLAUDE.md formula. (9) Fixed Q→D legacy mapping: DB still has "Q" from old ETL runs — frontend now maps Q→D for tooltips/labels/colors. 129/129 tests pass. Build OK (45 pages). Seeded 7 aliases, deployed. | Phase 10: wait for 14 days data |
| 26 | Feb 18, 2026 | Session 26 — Systematic Debugging + Full Verification. (1) verification-before-completion: 129/129 tests, 45/45 build, GitHub Actions 5/5 success, weights consistent. Found 3 stale formula docs — fixed. (2) systematic-debugging: Found CRITICAL bug in signal_detector.py — all 3 detectors referenced dead Polymarket data and Q component. Fixed: divergence now compares VI_trade vs DevAdoption momentum, momentum_breakout checks DevAdoption growth, quality_backed→adoption_backed uses D component. (3) Updated TECHNICAL_SPEC.md: 16 edits removing all Polymarket/Q/Arena Elo refs. (4) Updated roadmap.html Phase 10+: 14 Polymarket→DevAdoption replacements. (5) New tests: 133/133 pass (4 new: devadoption source verification, backward compat alias). Build OK. | Phase 11 or wait for data |
| 27 | Feb 18, 2026 | Session 27 — Phase 11: Fix Conversion Funnel & UX (8 tasks). Tasks 11.2+11.3 already done (dashboard open, AuthButton shows real plan). Implemented: (11.1) Analytics — GA4+Clarity scripts in Analytics.tsx, trackConversion() utility, events on pricing page. (11.4) Upsell banners — useUserPlan hook, UpsellBanner component (3 variants: dashboard-delay, model-breakdown, compare-signals), integrated into DashboardView, ModelDetailView (BreakdownRadar blur now plan-based), CompareView. (11.6) CTA rewrite — Inception formula: "Stop guessing which AI is hot. Start knowing.", "See what's trending now", "Start your free trial", "For teams that can't afford to guess". (11.8) Enterprise contact form — ContactForm component + /api/contact route + contact_requests migration. (11.7) Annual plans + 7-day trial — PLANS expanded to 4 keys (monthly+annual), resolvePlanKey() for backward compat, trial_period_days in Stripe checkout, pricing page monthly/annual toggle with savings badges. (11.5) Post-checkout onboarding — WelcomeModal (3-step tour), integrated in DashboardView via ?welcome=true query param. 7 new files, 10 modified. Build OK (46/46 pages). | Run contact_requests migration in Supabase, set GA4/Clarity IDs, Stripe live mode |
| 28 | Feb 18, 2026 | Session 28 — Phase 12 GTM: Task 12.1 Newsletter Email Capture ✅. (1) Created newsletter_subscribers table migration (008_newsletter_subscribers.sql) — UUID id, email UNIQUE, source, subscribed_at, is_active + RLS + indexes. Ran in Supabase SQL Editor — success. (2) NewsletterSignup.tsx component with 2 variants: inline (card with heading for landing/pricing) and banner (slim strip for dashboard). Pattern matches ContactForm.tsx. (3) /api/newsletter/subscribe route — email validation, upsert with on_conflict:email, service_role client. (4) Added newsletter_signup + share_clicked to analytics.ts ConversionEvent. (5) Placements: landing page after Two Modes section, dashboard banner for anon/free users, pricing page after FAQ before ContactForm. Build OK (47/47 pages). Task 12.3 Blog ✅: installed next-mdx-remote + @tailwindcss/typography + gray-matter. Created blog.ts (getAllPosts, getPostBySlug, calculateReadTime), /blog listing page (ISR 1h), /blog/[slug] post page (MDXRemote, prose-invert, generateStaticParams, SEO metadata). Seed post: "What Is the AI Virality Index?" (~600 words). Added /blog to header nav. Updated tailwind config (typography plugin + content path). Build OK (49/49 pages). Task 12.7 Viral Loops ✅: ShareButtons.tsx (X, LinkedIn, Copy Link + trackConversion), /api/og edge route (dynamic OG image 1200x630 with model score+delta), integrated ShareButtons into ModelDetailView header + blog post header, updated models/[slug] OG metadata to use /api/og?slug=, added "Add AVI to Your Site" embed promo section on landing page. Build OK (49/49 pages). Task 12.8 $7 Weekly Report ✅: installed @react-pdf/renderer. Created report-pdf.tsx (2-page dark-theme PDF: overview with scores+movers, component breakdown+signals), /report landing page ($7 CTA + Stripe checkout), /report/success (download after payment), /api/stripe/report-checkout (Stripe one-time mode:payment), /api/report/latest (PDF generation + download). Migration 009_weekly_reports.sql ran in Supabase. Updated webhook to skip plan upgrade for report purchases. Added report CTA to landing page and dashboard. Build OK (53/53 pages). Task 12.9 Email Digest ✅: Created email_digest.py (queries Supabase REST API, renders Jinja2, sends via Resend), weekly_digest.html template (dark theme email-safe HTML with scores table, overview cards, CTA), /api/newsletter/unsubscribe route (HMAC-signed token verification), .github/workflows/weekly_digest.yml (cron Monday 13:00 UTC). Updated config.py (RESEND_API_KEY), requirements.txt (jinja2). Build OK (54/54 pages). All 5 code tasks done. | Git push, env vars needed: RESEND_API_KEY, UNSUBSCRIBE_SECRET |
| 29 | Feb 19, 2026 | Session 29 — Component Breakdown Bug Fixes ✅. (1) Bug 1: Deleted 21 legacy Q (Arena Elo) component records from component_scores in Supabase — Q was replaced by D (Dev Adoption) in Phase 9 but old records persisted, causing inconsistent component names on radar charts. (2) Bug 2: Fixed normalizer returning 50.0 for zero-data components — changed _minmax_normalize and quantile_normalize to return 0.0 when all values are 0 (no signal) instead of 50.0 (misleading neutral). (3) Bug 3: Fixed index_calculator.py — changed default from 50 to 0 for components with empty history (models without data sources like Perplexity with no GitHub/DevAdoption). Also fixed component_breakdown JSON defaults and component_scores upsert defaults from 50→0. (4) Fixed frontend models/[slug]/page.tsx — increased component query limit from 12→42, guaranteed all 6 components (T,S,G,N,D,M) always shown via ALL_COMPONENTS array, smarter per-component date lookup instead of global latest day. (5) Updated existing DB data: SET normalized_value=0, smoothed_value=0 WHERE raw_value=0 AND normalized_value=50. Build OK (54/54 pages). | M (Mindshare) stays at 50 for all models until >7 days of Wikipedia data with variance accumulates |
| 30 | Feb 19, 2026 | Session 30 — Full Component Audit + Cross-Model Normalization ✅. (1) Root cause analysis: M=50 for all models because only 1-4 days of ETL data (started Feb 16). Per-model quantile normalization requires 7+ days; with <7 days, min-max fallback returns 50 when all identical or single-value. All 6 components affected (T,S,G,N,D,M). (2) Implemented cross_model_normalize() in normalizer.py — ranks all 7 models against each other when per-model history <7 days. Non-zero values scaled to 10-100 range using min-max across models. Zero-value models get 0. (3) Rewrote run_daily_calculation() in index_calculator.py with 3-phase approach: Phase 1=per-model quantile (existing), Phase 2=cross-model ranking for sparse components, Phase 3=merge results, recalculate VI scores with corrected values, write to DB. Added _needs_cross_model() helper. (4) Cleaned up dead data: deleted polymarket records from raw_metrics (100% zeros), kept arena for Elo badge metadata only. (5) Ran local ETL — all 7/7 models calculated. Results now differentiated: ChatGPT 95.8/94.1, Gemini 34.5/46.2, Claude 30.4/34.7, Grok 22.5/30.7, Perplexity 21.6/29.0, Copilot 13.2/18.3, DeepSeek 9.9/9.2. (6) Verified component_scores in Supabase: D component ranges 0-100 (was all 50), M ranges 10-100 (was all 50), all 6 components differentiated. 133/133 tests pass. Build OK (54/54 pages). Commits: 5daf803 + 16027e5. | Next GH Actions cron will use new code; ISR cache will refresh within 1h after |
| 31 | Feb 26, 2026 | Session 31 — llms.txt + UX improvements ✅. (1) llms.txt for AI Visibility: brainstorming (3 approaches), chose static manifest + dynamic data. Created web/public/llms.txt (manifest) + web/src/app/llms-full.txt/route.ts (live scores from Supabase, ISR 1h). Updated sitemap.ts (+3 URLs) and robots.ts. Design doc: docs/plans/2026-02-19-llms-txt-design.md. Verified on production. (2) Polymarket research: analyzed 3 PDFs on AI prediction markets. 86 active AI markets on Polymarket ($50M+). Identified structural gap — no adoption-metric markets. Prepared 3 Manifold test markets (npm SDK battle, Google Trends race, GitHub stars). (3) Compare page: free users limited to 3 models, Pro gets all 7. Plan-aware toggle with upsell hint. (4) Global footer: added to layout.tsx (was only on landing page). Nav links + contact email getdroneservices@gmail.com. (5) Active Signals audit: only ChatGPT generates signals (adoption_backed, 4 signals). Other models don't pass thresholds yet (D<65 or delta7<=0). Commits: 60071d6 + 4874c67. | Manifold markets testing, Phase 10 data accumulation |
| 32 | Mar 15, 2026 | Session 32 — Phase 10 INDEX VALIDATION ✅ (tasks 10.1-10.4). 28 days of data (196 daily scores, 1155 component scores, 4615 raw metrics, 36 signals). Dispatched 4 parallel analysis agents: (1) Trends & Rankings — ChatGPT-Claude duopoly forming (gap 27→4 pts), content market deflating (all 7 models declining), Perplexity strongest acceleration. (2) Component Correlation — N leads S by 1-2 days (best predictor), G-D correlation +0.50 (redundant?), S is most volatile but highest weight (0.28). (3) Signal Quality — 36 signals, bullish adoption_backed hit rate 7% (terrible, fires at peaks), bearish divergence 64% (decent), 3/7 models get zero signals (D=0), momentum_breakout never fires. (4) Real-World Event Correlation — 9/10 major events captured: ChatGPT Agent launch Feb 19 (+54 pts), Claude #1 App Store, DeepSeek hype collapse (-39% content), xAI co-founder exodus, Perplexity Computer launch. INDEX VALIDATED. Data quality bugs found: DeepSeek D raw=158, negative GitHub deltas, Copilot/Grok/Perplexity D=0 structural gap. Full report: docs/analysis/2026-03-15-index-validation.md | Phase 10.5 weight calibration, 10.6 methodology whitepaper, 10.7 case studies, fix signal system |
| 33 | Mar 15, 2026 | Session 33 — Signal System Redesign (Task 10.8) ✅. Fixed 5 critical problems found in 28-day validation: (1) Replaced detect_adoption_backed with detect_mean_reversion — catches oversold models (delta7<-5) with strong fundamentals (D>40 or T>30/N>40 for non-SDK models), instead of buying peaks. (2) Fixed momentum_breakout thresholds: VI>55/d7>8 (was 70/15), removed accel>0 requirement that prevented all signals. (3) Added detect_trend_momentum for non-SDK models (Copilot/Grok/Perplexity with D=0) using T+S+N components — bullish when T growing + N>40 or S>30, bearish when declining + weak S+N. (4) Added _resolve_signal_conflicts — when same model gets bullish+bearish on same day, keeps only the stronger direction. (5) Updated run_signal_detection: 4 detectors, per-model conflict resolution before DB write. Updated tests: TestDetectMeanReversion (6 tests), TestDetectTrendMomentum (5 tests), TestResolveSignalConflicts (5 tests), updated TestDetectMomentumBreakout for new thresholds. Backward compat: detect_quality_backed = detect_mean_reversion alias kept. | Phase 10.5 weight calibration, 10.6 methodology whitepaper |
| 34 | Mar 15, 2026 | Session 34 — Phase 10 COMPLETE ✅ (tasks 10.5-10.8). (1) 10.5 Weight Calibration: S 0.28→0.20 (too volatile), D 0.15→0.20 (most stable), N 0.12→0.15 (best predictor), G 0.15→0.12 (correlated with D), M 0.12→0.15. Content: S 0.35→0.25, N 0.20→0.25, M 0.10→0.15. Updated config.py, index_calculator.py docstring, tests, CLAUDE.md, TECHNICAL_SPEC.md. (2) 10.6 Methodology Whitepaper: web/public/methodology.md — 9 sections covering data sources, formula with rationale, normalization pipeline, momentum, 4 signal types, validation results, limitations. (3) 10.7 Case Studies: 3 marketing cases — ChatGPT Agent launch (+54pts, decay pattern), Claude dethroning (Pentagon controversy → #1 App Store, duopoly formation), DeepSeek hype collapse (-39% content, 5-phase pattern). Files: docs/case-studies/. (4) Signal redesign already done in session 33. 145/145 tests pass. Build OK. Phase 10 = ГОТОВО. Roadmap 97%. | Phase 12 GTM remaining tasks, deploy |
| 35 | Mar 15, 2026 | Session 35 — HONEST ASSESSMENT: Index is reactive, not predictive ✅. Committed + pushed Phase 10 (24f3c4c). Analyzed DB state: 36 old signals (divergence + adoption_backed), new signal code (mean_reversion, trend_momentum, momentum_breakout) has 0 real-world firings — just deployed. Copilot/Grok/Perplexity have 0 signals ever. DeepSeek D normalization broken (raw=169, smoothed=80.3). Created 60-day validation checklist: docs/analysis/60-day-validation-checklist.md — defines Phase 1 (signal audit at day 42), Phase 2 (accuracy measurement at day 60, target >55%), Phase 3 (implement predictive patterns: decay predictor, news cascade, floor detector, zero-sum alert), Phase 4 (weight revalidation). Honest positioning: market as "AI attention tracker" not "predictive signals" until accuracy proven. | 60-day checkpoint ~Apr 16, Phase 12 GTM |
| 35b | Mar 15, 2026 | Session 35b — 5 BUG FIXES ✅. (1) D normalization threshold: raw < 1000 downloads/day → D=0 (fixes DeepSeek D=80 on 169 downloads). (2) GitHub negative deltas: clamped to max(0, delta) — stars can't decrease. (3) Non-SDK weight redistribution: Copilot/Grok/Perplexity now use proportionally redistributed weights (D=0, other weights increase to sum=1.0) instead of 20% dead weight. Added get_weights_for_model() to config.py. (4) Cleaned 17 old adoption_backed signals from DB. (5) Dry-ran signal detection: 4 signals fired — mean_reversion for ChatGPT+DeepSeek, divergence for Gemini, trend_momentum for Grok. First time Grok gets a signal! 154/154 tests pass (+9 new). | 60-day checkpoint, Phase 12 GTM |
| 36 | Mar 19, 2026 | Session 36 — MID-CYCLE ANALYSIS + ANALYTICS ✅. (1) Data analysis: 32 days of data (Feb 16 – Mar 19), 224 rows, 0 gaps. Rankings: ChatGPT #1 (48.97), Claude #2 (38.63), Copilot #3 (37.73). Trends: only Perplexity (+5.92) and Copilot (+3.84) trending up, all others declining. DeepSeek in freefall (-11.73). (2) Signal validation: 30 signals since Mar 15 — mean_reversion: 15, trend_momentum: 9, divergence: 6, momentum_breakout: 0 (still never fires). New signals ARE working! (3) Vercel Analytics: installed @vercel/analytics + @vercel/speed-insights alongside existing GA4/Clarity. Pushed 4c87a06. (4) Polymarket research: affiliate program pays $0.01/click + $10/first deposit ($20 min). Managed via Dub.co, payout via Stripe at $100 threshold. Uncapped referrals. Potential integration: add Polymarket links to AI-related markets from dashboard. | momentum_breakout threshold review, Polymarket integration, Phase 12 GTM |
| 37 | Mar 19, 2026 | Session 37 — DEEP QUALITY AUDIT + V0.2 PLAN ✅. (1) Brutal accuracy analysis: signal hit rate 48.1% (coin flip — divergence 55%, mean_reversion 40%, trend_momentum 0%). Ranking stability: #1 changes 1x/week (25.8%). Component reliability: D=45% zeros, G=37% zeros, max daily swings 100pts in G/M. Two modes 92.4% correlated — redundant. Real-world check: Copilot #3 above Gemini is wrong (D=0/G=0 inflation). (2) Competitive research: NO direct competitor. Nearest: aifearandgreed.com (primitive), Artificial Analysis (benchmarks only, free+enterprise). LMSYS Arena (academic, free). SimilarWeb ($199+/mo). Our niche "Fear & Greed for AI" is empty. (3) Polymarket podcast analysis (Dollar Den interview): ecosystem of wrappers/terminals growing (TradeFox, PMX, PolyClue), AI models for probability calculation, "capper" business model. Our VI = data signal for AI event predictions. (4) Created v0.2 improvement plan (7 phases): kill two modes, 5-component formula (drop D), rewrite signals to spike/drop/rank_change, versioning, new pricing ($19 Pro), Polymarket integration roadmap. Plan approved, saved to plans/typed-honking-patterson.md. | Execute v0.2 plan: Phase 1 (kill modes) + Phase 4 (versioning) first |
| 38 | Mar 19, 2026 | Session 38 — V0.2 PHASE 1 + PHASE 4 ✅. **Phase 1 (Remove lies from UI)**: (1.1) Deleted "Two Modes, One Index" section from landing page, removed ModeToggle from Dashboard/Compare/Model pages, deleted ModeToggle.tsx component. Hardcoded mode='trade' in all views. (1.2) Renamed "Trading Signals"→"Notable Changes", replaced "bullish"/"bearish" with "rising"/"declining" in ModelDetailView. Removed Pro-gate from signals API, added disclaimer to meta. (1.3) Updated pricing: Pro Trader $29 + Pro Builder $99 → Pro $19 + Team $79. Updated landing page: "5 Data Components" (removed D card), G label "Developer Adoption"→"GitHub Activity", hero "5 data components", How It Works card "activity alerts" instead of "trading signals", new pricing preview. Updated pricing page FAQ. **Phase 4 (Versioning)**: Tagged v0.1.0 on baseline commit (ad9d5d9), package.json→0.2.0, "v0.2.0" in layout footer, created CHANGELOG.md (v0.2.0 + v0.1.0 entries). TypeScript: 0 errors. Build: only pre-existing supabaseUrl errors (no .env in worktree). | V0.2 Phase 2 (formula), Phase 3 (signals rewrite) |
| 38b | Mar 19, 2026 | Session 38b — V0.2 PHASE 2 + PHASE 3 ✅. **Phase 2 (5-component formula)**: Single WEIGHTS dict (0.25T+0.25S+0.10G+0.25N+0.15M), D dropped from formula (data still collected). WEIGHTS_TRADE/WEIGHTS_CONTENT aliased to WEIGHTS for backward compat. EWMA_ALPHA=0.30 (single). get_weights_for_model() now redistributes G (not D) for models without GitHub. index_calculator.py: single calculate_index (mode param ignored), run_daily_calculation writes vi_trade=vi_content=VI. _model_has_sdk→_model_has_github. **Phase 3 (honest signals)**: Complete rewrite of signal_detector.py — 3 types (spike/drop/rank_change) using z-score of delta7 series. Direction: "rising"/"declining" (no bullish/bearish). No conflict resolution needed (types are mutually exclusive). Updated CLAUDE.md, TECHNICAL_SPEC.md. 133/133 tests pass. TypeScript: 0 errors. | V0.2 Phases 5+6 (pricing, landing cleanup) |
| 38c | Mar 19, 2026 | Session 38c — V0.2 PHASES 5+6 ✅. **Phase 5.1**: stripe.ts updated — Pro $19/mo ($169/yr), Team $79/mo ($699/yr). Legacy plan key resolver for backward compat. **Phase 5.2**: accuracy_tracker.py — resolves signal outcomes 7 days after expiry, tracks vi_at_signal vs vi_at_outcome, get_rolling_accuracy(90 days). Migration 010_signal_outcomes.sql. **Phase 6**: Already done in Phase 1 — verified all 4 items (5 components, hero cleanup, pricing preview, activity alerts). Verification: 133/133 tests pass, build compiles OK, git tags confirmed. CHANGELOG.md updated with full v0.2 changes. **V0.2 PLAN COMPLETE** (Phases 1-6). Phase 7 (Polymarket) is future work (after 90 days data). | Push to main, deploy, run migration |
| 38d | Mar 19, 2026 | Session 38d — V0.2 DEPLOY ✅. Merged claude/goofy-liskov → main (3 commits, -1253 lines net). Pushed to GitHub → Vercel auto-deploy triggered. Ran migration 010_signal_outcomes.sql in Supabase SQL Editor — SUCCESS. Verified live site: landing page (5 components, no Trading/Building), dashboard (single score, no ModeToggle, 7 models), footer v0.2.0. **V0.2.0 FULLY DEPLOYED.** | Next check: Mar 26 (accuracy tracker data, signal quality, score sanity) |
